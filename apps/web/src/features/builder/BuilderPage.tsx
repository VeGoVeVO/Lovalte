...
const fromTemplate = (tmpl: CardTemplateDTO): Form => {
  ...
  return {
    name: tmpl.name, orgName: b.organizationName, logoText: b.logoText ?? "",
    ...
    ppv: tmpl.rewardRule.pointsPerVisit, rThreshold: tmpl.rewardRule.rewardThreshold,
...
export function BuilderPage() {
  ...
  const openEdit = (card: CardTemplateDTO) => { setForm(fromTemplate(card)); setEditing(card); setStatus(null); };
  ...
  const save = async () => {
    ...
    try {
      let id: string;
      if (editing === "new") {
        const tmpl = await createMut.mutateAsync(toInput(form)); id = tmpl.id; setEditing(tmpl);
      } else {
        const tmpl = await updateMut.mutateAsync({ id: existing!.id, input: toInput(form) }); id = tmpl.id; setEditing(tmpl);
      }
      const assets = ([
  ...
  const publish = async () => {
    ...
    if (!id) { setStatus(t("Save the draft first.")); setAwaitConfirm(false); return; }
    try {
      await publishMut.mutateAsync(id); setStatus(t("Published.")); closeEdit();
    } catch (err) {
      setStatus((err as { message?: string })?.message ?? t("Publish failed.")); setAwaitConfirm(false);
    }
  };
    ...
    return (
      <AppShell title={t("Card Builder")}>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "1.5rem" }}>
          <GlassButton type="button" onClick={openNew}>{t("+ New card")}</GlassButton>
        </div>
        {templates.isLoading && <p className="body" aria-live="polite">{t("Loading templates…")}</p>}
        {templates.isError && <GlassCard className="feature"><p className="body" role="alert">{t("Could not load templates. Please refresh.")}</p></GlassCard>}
        {!templates.isLoading && !templates.isError && list.length === 0 && (
          <GlassCard className="feature"><p className="body">{t("No templates yet - create your first loyalty card.")}</p></GlassCard>
        )}
        {list.length > 0 && (
  ...
  return (
    <AppShell title={saved ? t("Edit: {name}", { name: saved.name }) : t("New card")}>
      <div style={{ marginBottom: "1rem" }}>
        <GlassButton type="button" variant="ghost" onClick={closeEdit} aria-label={t("Back to templates list")}>{t("← Back")}</GlassButton>
      </div>
      <div className="grid-3" style={{ gridTemplateColumns: "1fr 1fr", gap: "2rem", alignItems: "start" }}>
        ...
        <GlassCard className="feature" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <h2 className="cardt" id="editor-heading">{t("Card settings")}</h2>

          <div><Lbl htmlFor="f-name" text={t("Template name *")} />
            <GlassInput id="f-name" value={form.name} placeholder={t("e.g. Summer Campaign")} maxLength={100}
              required aria-required="true" onChange={(e: CE) => patch({ name: e.target.value })} /></div>

          <div><Lbl htmlFor="f-org" text={t("Business name *")} />
            <GlassInput id="f-org" value={form.orgName} placeholder={t("Shown on the card")} maxLength={64}
              required aria-required="true" onChange={(e: CE) => patch({ orgName: e.target.value })} /></div>

          <div><Lbl htmlFor="f-logo" text={t("Logo text (max 24 chars)")} />
            <GlassInput id="f-logo" value={form.logoText} placeholder={t("e.g. LOYALTY")} maxLength={24}
              onChange={(e: CE) => patch({ logoText: e.target.value })} /></div>

          <fieldset style={{ border: "none", margin: 0, padding: 0 }}>
            <legend className="eyebrow" style={{ marginBottom: "0.5rem" }}>{t("Colors")}</legend>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.6rem" }}>
              {colorFields.map(({ label, id, field }) => (
                <div key={id}>
                  <label htmlFor={`cp-${id}`} className="eyebrow" style={{ display: "block", marginBottom: "0.3rem" }}>{t(label)}</label>
                  <ColorPicker id={`cp-${id}`} ariaLabel={t("{label} color", { label: t(label) })} value={form[field] as string}
          ...
          <fieldset style={{ border: "none", margin: 0, padding: 0 }}>
            <legend className="eyebrow" style={{ marginBottom: "0.5rem" }}>{t("Primary field *")}</legend>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
              <GlassInput value={form.pLabel} placeholder={t("Label (e.g. POINTS)")} aria-label={t("Primary field label")}
                onChange={(e: CE) => patch({ pLabel: e.target.value })} />
              <GlassInput value={form.pValue} placeholder={t("Template (e.g. {{points}})")} aria-label={t("Primary field value template")}
                onChange={(e: CE) => patch({ pValue: e.target.value })} />
            </div>
          </fieldset>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <div><Lbl htmlFor="f-ppv" text={t("Points per visit")} />
              <GlassInput id="f-ppv" type="number" min={1} value={form.ppv}
                onChange={(e: CE) => patch({ ppv: Math.max(1, Number(e.target.value)) })} /></div>
            <div><Lbl htmlFor="f-rth" text={t("Reward threshold")} />
          ...
          <fieldset style={{ border: "none", margin: 0, padding: 0 }}>
            <legend className="eyebrow" style={{ marginBottom: "0.75rem" }}>{t("Card images")}</legend>
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <AssetField
                kind="icon" label={t("Icon *")} hint={t("Required to publish. Pick a Lucide icon or upload a 29×29 px PNG.")}
                value={form.iconRef} iconColor={toRgb(form.fgHex)}
                onChange={(url) => patch({ iconRef: url })} />
              <AssetField
                kind="logo" label={t("Logo")} hint={t("Shown top-left on the pass. Upload ≤160×50 px PNG.")}
                value={form.logoRef} onChange={(url) => patch({ logoRef: url })} />
              <AssetField
                kind="strip" label={t("Strip")} hint={t("Full-width banner. Upload 375×144 px PNG.")}
          ...
          <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", marginTop: "0.25rem" }}>
            ...
            {saved && !isPublished && (
              <GlassButton type="button" variant="ghost" onClick={publish} disabled={publishMut.isPending} aria-busy={publishMut.isPending}>
                {publishMut.isPending ? t("Publishing…") : awaitConfirm ? t("Confirm publish?") : t("Publish")}
              </GlassButton>
            )}
            {awaitConfirm && (
              <GlassButton type="button" variant="ghost" onClick={() => setAwaitConfirm(false)}>{t("Cancel")}</GlassButton>
            )}
          </div>
          ...
          {isPublished && (
            <p className="body" style={{ color: "var(--muted)" }}>
              {t("Published (v{version}). Saving will create a new draft version.", { version: saved!.version })}
            </p>
          )}
        </GlassCard>
        ...
        <div style={{ position: "sticky", top: "6rem", display: "flex", flexDirection: "column", gap: "1rem", alignItems: "center" }}>
          <p className="eyebrow" style={{ textAlign: "center" }}>{t("Live preview")}</p>
          <CardPreview
